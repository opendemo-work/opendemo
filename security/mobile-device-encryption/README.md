...
> - 生产环境执行前请经过变更评审和备份确认。
```bash
cd security/mobile-device-encryption
./scripts/start.sh
./scripts/check.sh
```
...
iOS 数据保护 是 移动设备加密 的基础，正确理解和配置它是保障安全的前提。

### 2. Android FBE

Android FBE 直接影响系统的安全性和可用性，需要根据组织策略进行规划。

### 3. MDM

MDM 提供了关键的技术能力，支持安全机制的有效运行。

### 4. 移动安全

移动安全 关系到合规性和审计要求，是企业安全治理的重要组成部分。
...
Mobile Device Encryption 的核心机制可以概括为以下几个步骤：
