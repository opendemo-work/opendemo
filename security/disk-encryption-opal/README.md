...
> 演示基于 TCG Opal 标准的自加密硬盘（SED）配置和管理。
...
> - 生产环境执行前请经过变更评审和备份确认。
```bash
cd security/disk-encryption-opal
./scripts/start.sh
./scripts/check.sh
```
...
TCG Opal 是 Opal 磁盘加密 的基础，正确理解和配置它是保障安全的前提。

### 2. SED

SED 直接影响系统的安全性和可用性，需要根据组织策略进行规划。

### 3. 自加密硬盘

自加密硬盘 提供了关键的技术能力，支持安全机制的有效运行。

### 4. PSID Revert

PSID Revert 关系到合规性和审计要求，是企业安全治理的重要组成部分。
...
sedutil-cli --setLockingRange 0 Lock /dev/nvme0 password
```
...
Disk Encryption with OPAL 的核心机制可以概括为以下几个步骤：
