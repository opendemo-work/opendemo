...
HSM 是 HSM 硬件安全模块基础 的基础，正确理解和配置它是保障安全的前提。

### 2. FIPS 140-2

FIPS 140-2 直接影响系统的安全性和可用性，需要根据组织策略进行规划。

### 3. 密钥不可导出

密钥不可导出 提供了关键的技术能力，支持安全机制的有效运行。

### 4. PKCS#11

PKCS#11 关系到合规性和审计要求，是企业安全治理的重要组成部分。
...
> - 生产环境执行前请经过变更评审和备份确认。
```bash
# HSM 操作通常通过厂商 SDK 或 PKCS#11 接口
pkcs11-tool --module libCryptoki2_64.so --list-slots
```
