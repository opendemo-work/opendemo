...
> 演示 Linux 全盘加密系统启动时通过 SSH 远程输入密码解锁。
...
```bash
cd security/luks-remote-unlock
./scripts/start.sh
./scripts/check.sh
```
...
Dropbear 是 LUKS 远程解锁 的基础，正确理解和配置它是保障安全的前提。

### 2. initramfs

initramfs 直接影响系统的安全性和可用性，需要根据组织策略进行规划。

### 3. SSH 远程解锁

SSH 远程解锁 提供了关键的技术能力，支持安全机制的有效运行。

### 4. LUKS

LUKS 关系到合规性和审计要求，是企业安全治理的重要组成部分。
...
```bash
# 配置 dropbear 到 initramfs
update-initramfs -u
# 启动后通过 SSH 连接并输入 cryptroot-unlock
```
...
LUKS Remote Unlock 的核心机制可以概括为以下几个步骤：
