...
> 演示 macOS FileVault 磁盘加密的配置、密钥托管和恢复流程。
...
> - 生产环境执行前请经过变更评审和备份确认。
```bash
cd security/filevault-management
./scripts/start.sh
./scripts/check.sh
```
...
FileVault 是 FileVault 管理 的基础，正确理解和配置它是保障安全的前提。

### 2. 个人恢复密钥

个人恢复密钥 直接影响系统的安全性和可用性，需要根据组织策略进行规划。

### 3. 机构恢复密钥

机构恢复密钥 提供了关键的技术能力，支持安全机制的有效运行。

### 4. fdesetup

fdesetup 关系到合规性和审计要求，是企业安全治理的重要组成部分。
...
FileVault Management 的核心机制可以概括为以下几个步骤：
