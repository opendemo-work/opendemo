...
> 演示使用 Azure Disk Encryption 和 Azure Key Vault 加密虚拟机磁盘。
...
> - 生产环境执行前请经过变更评审和备份确认。
```bash
cd security/cloud-disk-encryption-azure
./scripts/start.sh
./scripts/check.sh
```
...
Azure Key Vault 是 Azure 云磁盘加密 的基础，正确理解和配置它是保障安全的前提。

### 2. Disk Encryption

Disk Encryption 直接影响系统的安全性和可用性，需要根据组织策略进行规划。

### 3. VM Extension

VM Extension 提供了关键的技术能力，支持安全机制的有效运行。

### 4. ADE

ADE 关系到合规性和审计要求，是企业安全治理的重要组成部分。
...
Cloud Disk Encryption Azure 的核心机制可以概括为以下几个步骤：
