...
> 演示使用 GCP Customer-Managed Encryption Keys (CMEK) 加密 Persistent Disk。
...
> - 生产环境执行前请经过变更评审和备份确认。
```bash
cd security/cloud-disk-encryption-gcp
./scripts/start.sh
./scripts/check.sh
```
...
CMEK 是 GCP 云磁盘加密 的基础，正确理解和配置它是保障安全的前提。

### 2. Cloud KMS

Cloud KMS 直接影响系统的安全性和可用性，需要根据组织策略进行规划。

### 3. Persistent Disk

Persistent Disk 提供了关键的技术能力，支持安全机制的有效运行。

### 4. 默认加密

默认加密 关系到合规性和审计要求，是企业安全治理的重要组成部分。
...
gcloud compute disks create my-disk --zone us-central1-a --kms-key projects/PROJECT/locations/us-central1/keyRings/my-keyring/cryptoKeys/my-key
```
...
Cloud Disk Encryption Gcp 的核心机制可以概括为以下几个步骤：
