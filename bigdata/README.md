...
> 从消息队列到实时计算、从数据仓库到数据湖的完整大数据学习路径，所有案例均可一键本地运行。

---

## 🎯 学习目标

本技术栈帮助你掌握企业级大数据平台的核心组件：

- ✅ 使用 Kafka 构建高吞吐消息流管道
- ✅ 使用 Spark 进行批处理与流式计算
- ✅ 使用 Flink 实现低延迟实时 ETL
- ✅ 使用 ClickHouse 进行 OLAP 分析
- ✅ 理解数据仓库分层（ODS/DWD/DWS/ADS）设计
- ✅ 使用 Airflow/dbt 编排数据管道与数据转换
...
| 方向 | 案例数 | 状态 |
|------|--------|------|
| Kafka | 1 | ✅ kafka-stream-processing |
| Spark | 0 | 待补齐 |
| Flink | 0 | 待补齐 |
| ClickHouse | 0 | 待补齐 |
| 数据仓库 | 0 | 待补齐 |
...
### 路径 1：实时流处理工程师
```
Kafka → Flink → Kafka Streams → ClickHouse
```

### 路径 2：离线数据工程师
```
SQL → Spark SQL → Airflow → 数据仓库分层
```

### 路径 3：大数据架构师
```
Kafka → Spark/Flink → 数据湖 → OLAP → 数据治理
```
...
新增大数据案例需满足五星标准，并提供 docker-compose 或本地可运行脚本。
