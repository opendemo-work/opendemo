...
> 使用 Wireshark 捕获和分析网络数据包，定位网络问题。
...
```bash
cd networking/wireshark-packet-analysis
./scripts/start.sh
./scripts/check.sh
```
...
抓包 是 Wireshark 抓包分析 的基础，深入理解它有助于掌握整个网络机制。

### 2. 显示过滤器

显示过滤器 决定了网络的性能、可靠性和安全性，是设计和排障的关键。

### 3. 协议分层

协议分层 提供了关键的技术能力，支持复杂网络场景的实现。

### 4.  Follow TCP Stream

 Follow TCP Stream 是进阶内容，掌握它可以解决更实际的网络问题。
...
wireshark-packet-analysis 的核心机制可以概括为以下几个步骤：
