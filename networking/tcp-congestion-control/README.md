...
```bash
cd networking/tcp-congestion-control
./scripts/start.sh
./scripts/check.sh
```
...
拥塞窗口 是 TCP 拥塞控制 的基础，深入理解它有助于掌握整个网络机制。

### 2. 慢启动

慢启动 决定了网络的性能、可靠性和安全性，是设计和排障的关键。

### 3. Cubic

Cubic 提供了关键的技术能力，支持复杂网络场景的实现。

### 4. BBR

BBR 是进阶内容，掌握它可以解决更实际的网络问题。
...
TCP Congestion Control 的核心机制可以概括为以下几个步骤：
