...
```bash
cd networking/network-troubleshooting
./scripts/start.sh
./scripts/check.sh
```
...
分层排查 是 网络故障排查 的基础，深入理解它有助于掌握整个网络机制。

### 2. ping

ping 决定了网络的性能、可靠性和安全性，是设计和排障的关键。

### 3. traceroute

traceroute 提供了关键的技术能力，支持复杂网络场景的实现。

### 4. tcpdump

tcpdump 是进阶内容，掌握它可以解决更实际的网络问题。
...
Network Troubleshooting 的核心机制可以概括为以下几个步骤：
