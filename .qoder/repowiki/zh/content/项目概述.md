...
项目目前主要支持Python语言，内置了51个高质量的Python教学Demo，覆盖了Python的核心知识点，包括基础语法、数据类型、函数与类、高级特性、并发编程、标准库、文件与IO、网络与数据库、调试与测试等多个方面。未来计划扩展支持Java等更多编程语言。
...
- [PROJECT_SUMMARY.md](file://PROJECT_SUMMARY.md#L125-L157)
...
- [PROJECT_SUMMARY.md](file://PROJECT_SUMMARY.md#L89-L98)
...
- **DemoManager**：负责Demo的加载、保存和管理
- **SearchEngine**：负责在Demo库中搜索和匹配
- **DemoGenerator**：协调AI服务生成新的Demo
- **DemoVerifier**：验证生成的Demo是否可执行
- **ContributionManager**：管理用户贡献的Demo
...
2. `get`函数首先调用`SearchEngine`在本地库中搜索匹配的Demo
3. 如果找到匹配项，调用`DemoManager`将Demo复制到输出目录
4. 如果未找到匹配项，调用`DemoGenerator`生成新的Demo
5. `DemoGenerator`调用`AIService`生成代码，然后调用`DemoManager`保存Demo
...
- [USAGE_GUIDE.md](file://USAGE_GUIDE.md#L244-L274)
...
**Section sources**
- [PROJECT_SUMMARY.md](file://PROJECT_SUMMARY.md#L184-L239)
- [USAGE_GUIDE.md](file://USAGE_GUIDE.md#L128-L207)
- [ABOUT.md](file://ABOUT.md#L121-L197)
...
- [USAGE_GUIDE.md](file://USAGE_GUIDE.md#L243-L274)
- [opendemo/services/config_service.py](file://opendemo/services/config_service.py#L16-L48)
...
贡献管理由`ContributionManager`类负责，主要功能包括：
...
- [USAGE_GUIDE.md](file://USAGE_GUIDE.md#L298-L316)
...
**Section sources**
- [PROJECT_SUMMARY.md](file://PROJECT_SUMMARY.md#L310-L320)
