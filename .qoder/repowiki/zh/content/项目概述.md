...
项目目前支持多种编程语言，包括Python、Go和Node.js。其中，内置了51个高质量的Python教学Demo，覆盖了Python的核心知识点；92个Go语言Demo，包含完整的DevOps/SRE支持；67个Node.js Demo，同样包含完整的DevOps/SRE支持。此外，项目还新增了2个Kubernetes Operator Framework相关的演示，使得总演示数量从247增加至249。未来计划扩展支持Java等更多编程语言。
...
Open Demo CLI 提供了四个主要的命令来满足开发者的需求：`search`、`get`、`new`、`config` 和 `check`。
...
`check` 命令用于执行项目的质量检查，包括单元测试和CLI功能测试。该命令会生成详细的检查报告，帮助开发者确保代码质量和功能完整性。
...
检查结果会保存在项目根目录的 `check` 目录下，包含JSON和Markdown格式的报告文件。
...
报告已保存: check/check_report_20260106_160502.json
查看完整报告: cat check/check_report_20260106_160502.md
```

这个命令会运行所有单元测试和CLI功能测试，并生成详细的检查报告，帮助开发者确保代码质量和功能完整性。
