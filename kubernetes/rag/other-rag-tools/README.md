...
本案例展示如何在 Kubernetes 集群中部署其他常见的 RAG (Retrieval-Augmented Generation) 工具，用于构建智能问答机器人和知识管理平台。

## 功能说明

**其他 RAG 工具**：在 Kubernetes 集群中部署其他常见的 RAG 工具，如 LangChain、LlamaIndex、Haystack 等
...
```bash
# 1. 配置密钥
kubectl create secret generic other-rag-tools-secrets \
...
kubectl get svc -n other-rag-tools-system
```
...
```bash
# 获取服务地址
OTHER_RAG_TOOLS_SERVICE_IP=$(kubectl get svc other-rag-tools-service -n other-rag-tools-system -o jsonpath='{.status.loadBalancer.ingress[0].ip}')

# 访问服务界面
echo "其他 RAG 工具界面地址: http://$OTHER_RAG_TOOLS_SERVICE_IP:8080"

# 测试 API
curl -X GET http://$OTHER_RAG_TOOLS_SERVICE_IP:8080/api/v1/health
```
...
2. 用户发送问题 → RAG 服务 → 向量存储检索 → 大语言模型生成 → 返回答案
