...
本目录包含Meta在MCP (Model Control Plane)领域的实践案例，展示如何在Kubernetes环境中部署和管理基于Meta技术栈的MCP系统。

## 案例列表

### 1. LLaMA 大模型 MCP 部署

**功能说明**：在Kubernetes集群中部署基于Meta LLaMA大模型的MCP系统
...
**配置文件**：
- `llama-mcp-deployment.yaml` - LLaMA MCP服务部署配置
- `llama-mcp-service.yaml` - LLaMA MCP服务暴露配置
- `configmap.yaml` - 配置管理
- `secret.yaml` - 密钥管理
- `rbac.yaml` - 权限控制
...
```bash
# 1. 配置密钥
kubectl create secret generic llama-mcp-secrets \
  --from-literal=api-key=YOUR_LLAMA_API_KEY \
  --namespace mcp-system

# 2. 部署MCP服务
kubectl apply -f llama-mcp-deployment.yaml
kubectl apply -f llama-mcp-service.yaml
...
```bash
# 获取服务地址
MCP_SERVICE_IP=$(kubectl get svc llama-mcp-service -n mcp-system -o jsonpath='{.status.loadBalancer.ingress[0].ip}')
