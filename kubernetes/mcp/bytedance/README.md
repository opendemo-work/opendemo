...
本目录包含字节跳动在 MCP (Model Control Plane) 领域的实践案例，展示如何在 Kubernetes 环境中部署和管理基于字节跳动技术栈的 MCP 系统。

## 案例列表

### 1. 豆包大模型 MCP 部署

**功能说明**：在 Kubernetes 集群中部署基于字节跳动豆包大模型的 MCP 系统
...
**配置文件**：
- `doubao-mcp-deployment.yaml` - 豆包 MCP 服务部署配置
- `doubao-mcp-service.yaml` - 豆包 MCP 服务暴露配置
- `configmap.yaml` - 配置管理
- `secret.yaml` - 密钥管理
- `rbac.yaml` - 权限控制
...
```bash
# 1. 配置密钥
kubectl create secret generic doubao-mcp-secrets \
  --from-literal=api-key=YOUR_DOBAO_API_KEY \
  --namespace mcp-system

# 2. 部署 MCP 服务
kubectl apply -f doubao-mcp-deployment.yaml
kubectl apply -f doubao-mcp-service.yaml
...
```bash
# 获取服务地址
MCP_SERVICE_IP=$(kubectl get svc doubao-mcp-service -n mcp-system -o jsonpath='{.status.loadBalancer.ingress[0].ip}')
