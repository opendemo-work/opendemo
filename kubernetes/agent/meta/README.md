...
本目录包含Meta在AI Agent领域的实践案例，展示如何在Kubernetes环境中部署和管理基于Meta技术栈的AI Agent系统。

## 案例列表

### 1. LLaMA 大模型 Agent 部署

**功能说明**：在Kubernetes集群中部署基于Meta LLaMA大模型的AI Agent服务
...
**配置文件**：
- `llama-agent-deployment.yaml` - LLaMA Agent服务部署配置
- `llama-agent-service.yaml` - LLaMA Agent服务暴露配置
- `configmap.yaml` - 配置管理
- `secret.yaml` - 密钥管理
- `rbac.yaml` - 权限控制
...
```bash
# 获取服务地址
AGENT_SERVICE_IP=$(kubectl get svc llama-agent-service -n agent-system -o jsonpath='{.status.loadBalancer.ingress[0].ip}')
