...
本目录包含阿里巴巴在 AI Agent 领域的实践案例，展示如何在 Kubernetes 环境中部署和管理基于阿里巴巴技术栈的 AI Agent 系统。

## 案例列表

### 1. 通义千问 Agent 部署

**功能说明**：在 Kubernetes 集群中部署基于通义千问的 AI Agent 服务
...
**配置文件**：
- `agent-deployment.yaml` - Agent 服务部署配置
- `agent-service.yaml` - Agent 服务暴露配置
- `configmap.yaml` - 配置管理
- `secret.yaml` - 密钥管理
- `rbac.yaml` - 权限控制
...
```bash
# 获取服务地址
AGENT_SERVICE_IP=$(kubectl get svc agent-service -n agent-system -o jsonpath='{.status.loadBalancer.ingress[0].ip}')
