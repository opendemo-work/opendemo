...
> 演示如何启用和配置数据库审计日志，追踪敏感操作，满足合规要求。
...
> - 生产环境执行前请经过变更评审和备份确认。
```bash
cd database/audit-logging-demo
./scripts/start.sh
sleep 20
./scripts/check.sh
```
...
审计策略 是 数据库审计日志 的基础，理解它有助于正确设计和使用数据库相关方案。

### 2. 日志轮转

日志轮转 决定了系统的性能、可用性和扩展能力，需要根据业务场景权衡选择。

### 3. 合规要求

合规要求 提供了关键的运维和管理能力，是生产环境不可或缺的组成部分。

### 4. 敏感操作追踪

敏感操作追踪 关系到系统的安全性和合规性，需要按照最佳实践进行配置。
