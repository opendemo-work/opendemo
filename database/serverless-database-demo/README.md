...
> 演示 Serverless 数据库特性，包括按需计费、自动扩缩容和事件驱动架构。
...
```bash
cd database/serverless-database-demo
./scripts/start.sh
sleep 20
./scripts/check.sh
```
...
按需计费 是 Serverless 数据库 的基础，理解它有助于正确设计和使用数据库相关方案。

### 2. 自动扩缩容

自动扩缩容 决定了系统的性能、可用性和扩展能力，需要根据业务场景权衡选择。

### 3. Aurora Serverless

Aurora Serverless 提供了关键的运维和管理能力，是生产环境不可或缺的组成部分。

### 4. PolarDB Serverless

PolarDB Serverless 关系到系统的安全性和合规性，需要按照最佳实践进行配置。
...
```bash
# 通过云厂商控制台或 API 创建 Serverless 数据库集群
```
...
Serverless数据库实践完整指南 的核心机制可以概括为以下几个步骤：
