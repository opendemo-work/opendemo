...
> 演示 MySQL 读写分离架构，包括主从复制、应用层路由和中间件方案。
...
> - 生产环境执行前请经过变更评审和备份确认。
```bash
cd database/read-write-splitting-demo
./scripts/start.sh
sleep 20
./scripts/check.sh
```
...
主从复制 是 数据库读写分离 的基础，理解它有助于正确设计和使用数据库相关方案。

### 2. 读写分离

读写分离 决定了系统的性能、可用性和扩展能力，需要根据业务场景权衡选择。

### 3. 延迟问题

延迟问题 提供了关键的运维和管理能力，是生产环境不可或缺的组成部分。

### 4. ProxySQL

ProxySQL 关系到系统的安全性和合规性，需要按照最佳实践进行配置。
