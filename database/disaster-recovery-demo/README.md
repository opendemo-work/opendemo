...
> 演示数据库灾难恢复策略，包括跨地域备份、RTO/RPO 设计和恢复演练。
...
```bash
cd database/disaster-recovery-demo
./scripts/start.sh
sleep 20
./scripts/check.sh
```
...
灾难恢复 是 数据库灾难恢复 的基础，理解它有助于正确设计和使用数据库相关方案。

### 2. 跨地域备份

跨地域备份 决定了系统的性能、可用性和扩展能力，需要根据业务场景权衡选择。

### 3. RTO/RPO

RTO/RPO 提供了关键的运维和管理能力，是生产环境不可或缺的组成部分。

### 4. 恢复演练

恢复演练 关系到系统的安全性和合规性，需要按照最佳实践进行配置。
