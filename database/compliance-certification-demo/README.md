...
> 演示数据库在等保、GDPR、SOC2 等合规场景下的配置要求和审计方法。
...
> - 生产环境执行前请经过变更评审和备份确认。
```bash
cd database/compliance-certification-demo
./scripts/start.sh
sleep 20
./scripts/check.sh
```
...
等保 是 数据库合规认证 的基础，理解它有助于正确设计和使用数据库相关方案。

### 2. GDPR

GDPR 决定了系统的性能、可用性和扩展能力，需要根据业务场景权衡选择。

### 3. SOC2

SOC2 提供了关键的运维和管理能力，是生产环境不可或缺的组成部分。

### 4. 数据分级分类

数据分级分类 关系到系统的安全性和合规性，需要按照最佳实践进行配置。
