...
> 演示数据库连接池的工作原理和配置优化，包括 HikariCP、Druid、PgBouncer。
...
> - 生产环境执行前请经过变更评审和备份确认。
```bash
cd database/connection-pool-demo
./scripts/start.sh
sleep 20
./scripts/check.sh
```
...
连接复用 是 数据库连接池 的基础，理解它有助于正确设计和使用数据库相关方案。

### 2. 池大小设置

池大小设置 决定了系统的性能、可用性和扩展能力，需要根据业务场景权衡选择。

### 3. 连接泄漏

连接泄漏 提供了关键的运维和管理能力，是生产环境不可或缺的组成部分。

### 4. HikariCP

HikariCP 关系到系统的安全性和合规性，需要按照最佳实践进行配置。
