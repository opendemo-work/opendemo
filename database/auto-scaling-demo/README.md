...
> 演示云数据库的自动扩缩容机制，包括 CPU/连接数触发条件和读写分离扩展。
...
垂直扩展 是 数据库自动扩缩容 的基础，理解它有助于正确设计和使用数据库相关方案。

### 2. 水平扩展

水平扩展 决定了系统的性能、可用性和扩展能力，需要根据业务场景权衡选择。

### 3. 读写分离

读写分离 提供了关键的运维和管理能力，是生产环境不可或缺的组成部分。

### 4. 自动伸缩策略

自动伸缩策略 关系到系统的安全性和合规性，需要按照最佳实践进行配置。
...
```bash
# 云厂商控制台或 Terraform 配置自动扩缩容规则
# 示例：CPU > 70% 持续 5 分钟触发扩容
```
...
数据库自动扩缩容配置完整指南 的核心机制可以概括为以下几个步骤：
