...
> 演示数据库层面的访问控制策略，包括用户权限管理、角色分配、最小权限原则和审计配置。
...
```bash
cd database/access-control-demo
./scripts/start.sh
sleep 20
./scripts/check.sh
```
...
用户与角色 是 数据库访问控制 的基础，理解它有助于正确设计和使用数据库相关方案。

### 2. 最小权限原则

最小权限原则 决定了系统的性能、可用性和扩展能力，需要根据业务场景权衡选择。

### 3. GRANT/REVOKE

GRANT/REVOKE 提供了关键的运维和管理能力，是生产环境不可或缺的组成部分。

### 4. 行级安全（RLS）

行级安全（RLS） 关系到系统的安全性和合规性，需要按照最佳实践进行配置。
