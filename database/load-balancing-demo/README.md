...
> 演示数据库读写分离和负载均衡方案，包括 ProxySQL、MaxScale、PgPool。
...
> - 生产环境执行前请经过变更评审和备份确认。
```bash
cd database/load-balancing-demo
./scripts/start.sh
sleep 20
./scripts/check.sh
```
...
读写分离 是 数据库负载均衡 的基础，理解它有助于正确设计和使用数据库相关方案。

### 2. ProxySQL

ProxySQL 决定了系统的性能、可用性和扩展能力，需要根据业务场景权衡选择。

### 3. 负载均衡

负载均衡 提供了关键的运维和管理能力，是生产环境不可或缺的组成部分。

### 4. 读库路由

读库路由 关系到系统的安全性和合规性，需要按照最佳实践进行配置。
...
数据库负载均衡配置完整指南 的核心机制可以概括为以下几个步骤：
